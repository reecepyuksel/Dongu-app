...
const ChatModal = ({ isOpen, onClose, itemId, itemTitle, partnerName, partnerId, deliveryMethods }) => {
    ...
    useEffect(() => {
        ...
        socket.on('newMessage', (msg) => {
            if ((msg.sender?.id === partnerId || msg.receiver?.id === partnerId) && (msg.item?.id === itemId || !msg.item)) {
                setMessages(prev => {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            if (socket && partnerId) {
                socket.emit('stopTyping', { toUserId: partnerId, itemId: itemId || 'direct' });
            }
    ...
    const handleTradeResponse = async (msgId, status) => {
        try {
            setSending(true);
            await api.post(`/messages/trade-offer/${msgId}/respond`, { status });
            showToast(`Takas teklifi ${status === 'accepted' ? 'kabul edildi 🎉' : 'reddedildi ❌'}`, 'success');
            await fetchMessages();
        } catch (err) {
            showToast(err.response?.data?.message || 'İşlem başarısız oldu.', 'error');
        } finally {
            setSending(false);
        }
    };
    ...
    const handleInputChange = (e) => {
        ...
        if (socket && partnerId) {
            socket.emit('typing', { toUserId: partnerId, itemId: itemId || 'direct' });
                                ...
                                {showProfile && (
                                    <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 p-4 z-50 animate-fade-in-down">
                                        <div className="flex items-center gap-3 mb-3">
                                            ...
                                            <div>
                                                <p className="font-bold text-slate-800">{partnerName || 'Kullanıcı'}</p>
                                                <div className="flex items-center gap-1 mt-0.5">
                                                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium">⭐ İyilik Puanı: 100</span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-3">
                            {loading ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center">
                                    ...
                                    <p className="text-slate-300 text-xs mt-1">İlk mesajı siz gönderin!</p>
                                </div>
                            ) : (
                                messages.map(msg => {
                                    ...
                                    if (msg.isTradeOffer) {
                                        return (
                                            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                                                <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm border-2 ${isMine ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-slate-50 text-slate-800'}`}>
                                                    <div className="flex items-center gap-2 mb-2 font-bold text-emerald-700">
                                                        <span className="text-lg">🔄</span> Takas Teklifi
                                                    </div>
                                                    <p className="font-medium mb-3 whitespace-pre-wrap">{msg.content}</p>
                                                    
                                                    {msg.tradeMediaUrl && (
                                                        <div className="mb-3 rounded-xl overflow-hidden shadow-sm border border-slate-200/50 bg-black/5">
                                                            {msg.tradeMediaUrl.match(/\.(mp4|webm|ogg)$/i) ? (
                                                                <video 
                                                                    src={msg.tradeMediaUrl} 
                                                                    controls 
                                                                    className="w-full max-h-48 object-cover" 
                                                                />
                                                            ) : (
                                                                <a href={msg.tradeMediaUrl} target="_blank" rel="noopener noreferrer">
                                                                    <img 
                                                                        src={msg.tradeMediaUrl} 
                                                                        alt="Takas Sunulan Medya" 
                                                    ...
                                                    {msg.tradeStatus === 'pending' ? (
                                                        !isMine ? (
                                                            <div className="flex gap-2 mt-2">
                                                                <button onClick={() => handleTradeResponse(msg.id, 'accepted')} disabled={sending} className="flex-1 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition">Kabul Et</button>
                                                                <button onClick={() => handleTradeResponse(msg.id, 'rejected')} disabled={sending} className="flex-1 py-1.5 bg-red-100 text-red-700 rounded-lg text-xs font-bold hover:bg-red-200 transition">Reddet</button>
                                                            </div>
                                                        ) : (
                                                            <div className="text-xs text-emerald-600 font-semibold text-center py-1 mt-2 bg-emerald-100 rounded-lg">Cevap Bekleniyor</div>
                                                        )
                                                    ) : (
                                                        <div className={`text-xs font-bold text-center py-1.5 mt-2 rounded-lg 
                                                            ${msg.tradeStatus === 'accepted' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}
                                                        `}>
                                                            {msg.tradeStatus === 'accepted' ? '✅ Kabul Edildi' : '❌ Reddedildi'}
                                                        </div>
                                                    )}
                                                    
                                                    <p className={`text-[10px] mt-2 opacity-60 text-right`}>
                                                        {formatDistanceToNow(new Date(msg.createdAt.endsWith('Z') ? msg.createdAt : msg.createdAt + 'Z'), { addSuffix: true, locale: tr })}
                                                    </p>
                                                </div>
                                            </div>
                                        );
                                    ...
                                    return (
                                        <div key={msg.id} className={`flex ${isSystem ? 'justify-center' : isMine ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm ${isSystem
                                                ? 'bg-amber-50 text-amber-800 border border-amber-200 text-center w-full mx-4'
                                                : isMine
                                                    ? 'bg-emerald-600 text-white rounded-br-md'
                                                    : 'bg-slate-100 text-slate-700 rounded-bl-md'
                                                }`}>
                                                {isSystem && <span className="block mb-1 text-[10px] uppercase tracking-wider opacity-70 font-bold">📢 Sistem Mesajı</span>}
                                                <p>{msg.content}</p>
                                                <p className={`text-[10px] mt-1 ${isSystem ? 'text-amber-600' : isMine ? 'text-emerald-200' : 'text-slate-400'}`}>
                                                    {formatDistanceToNow(new Date(msg.createdAt.endsWith('Z') ? msg.createdAt : msg.createdAt + 'Z'), { addSuffix: true, locale: tr })}
                                                </p>
                                            </div>
                                        </div>
