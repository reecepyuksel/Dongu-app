...
const DeliveryConfirmModal = ({ isOpen, onClose, itemId, onSuccess }) => {
    ...
    const confirmDelivery = async () => {
        ...
        try {
            ...
            showToast('Harika! Teslimat onaylandı ve +100 İyilik Puanı kazandın. 🎉', 'success');
            onSuccess();
            onClose();
        } catch (error) {
            showToast(error.response?.data?.message || 'Onaylanırken bir hata oluştu.', 'error');
        } finally {
            setLoading(false);
        }
    };
                ...
                >
                    ...
                    <div className="bg-emerald-500 p-6 text-center relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <h2 className="text-2xl font-bold text-white font-[Outfit]">Mutlu Son!</h2>
                            <p className="text-emerald-50 text-sm mt-1">Eşyanızı teslim aldınız, harika!</p>
                        </div>
                    </div>
                    ...
                    <div className="p-6">
                        <div className="mb-6 space-y-3">
                            <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl flex gap-3 text-sm text-amber-800 font-medium">
                                ...
                                <p>Onay işlemiyle birlikte hem paylaşım sahibi hem de sana <strong>+100 İyilik Puanı</strong> anında eklenecek.</p>
                            </div>
                            <p className="text-sm text-slate-500 font-medium text-center">
                                İsteğe bağlı olarak bu güzel anı bir fotoğrafla ölümsüzleştirebilirsin! (Döngü Kanıtı)
                            </p>
                        </div>
                            ...
                            {!preview ? (
                                <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-slate-200 border-dashed rounded-2xl cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition group">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p className="mb-1 text-sm font-bold text-slate-600">Mutlu Son Fotoğrafı Yükle</p>
                                        <p className="text-xs text-slate-400">Tıkla veya sürükle (İsteğe Bağlı)</p>
                                    </div>
                                ...
                                <div className="relative w-full h-48 rounded-2xl overflow-hidden group border border-slate-200 shadow-sm">
                                    <img src={preview} alt="Kanıt Önizleme" className="w-full h-full object-cover" />
