...
import { X, Upload, Loader2, Image as ImageIcon, ChevronRight, ChevronLeft, CheckCircle2 } from 'lucide-react';
import api from '../api';
...
const createItemSchema = z.object({
    ...
    deliveryMethods: z.array(z.string()).min(1, 'En az bir teslimat yöntemi seçmelisiniz'),
...
const CreateItemModal = ({ isOpen, onClose, onItemCreated }) => {
    ...
    const selectedMethods = watch('deliveryMethods');
    ...
    const handleNext = async () => {
        if (step === 1) {
            ...
            const isValid = await trigger(['title', 'category', 'description', 'city', 'district', 'neighborhood']);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            formData.append('selectionType', data.shareType === 'exchange' ? 'manual' : data.selectionType);
            ...
            console.error(err);
            showToast(err.response?.data?.message || 'Paylaşım başlatılırken bir hata oluştu, lütfen tekrar deneyin.', 'error');
        } finally {
                    ...
                    <div className="relative border-b border-slate-100 flex-shrink-0">
                        ...
                        <div className="flex items-center justify-between px-6 py-5">
                            <div className="flex flex-col">
                                ...
                                <p className="text-sm text-slate-500 font-medium">Adım {step} / 3</p>
                            </div>
                            <button
                                onClick={handleClose}
                        ...
                        {step === 1 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                ...
                                <div className="space-y-3 mb-6">
                                    <label className="block text-base font-bold text-slate-800 text-center mb-2">Paylaşım Türünü Seçin</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <label className={`flex flex-col items-center justify-center p-5 border-2 rounded-[20px] cursor-pointer transition-all ${shareType === 'donation' ? 'border-emerald-500 bg-emerald-50/50 shadow-md ring-4 ring-emerald-500/10' : 'border-slate-200 hover:border-emerald-200 bg-slate-50 hover:bg-white'}`}>
                                            <input type="radio" value="donation" {...register('shareType')} className="sr-only" />
                                            <div className="text-4xl mb-3">🎁</div>
                                            <span className="text-base font-bold text-slate-800">Döngüye Kat</span>
                                            <span className="text-xs text-slate-500 font-medium mt-1">Karşılıksız iyilik yap</span>
                                        </label>
                                        <label className={`flex flex-col items-center justify-center p-5 border-2 rounded-[20px] cursor-pointer transition-all ${shareType === 'exchange' ? 'border-emerald-500 bg-emerald-50/50 shadow-md ring-4 ring-emerald-500/10' : 'border-slate-200 hover:border-emerald-200 bg-slate-50 hover:bg-white'}`}>
                                            <input type="radio" value="exchange" {...register('shareType')} className="sr-only" />
                                            <div className="text-4xl mb-3">🔄</div>
                                            <span className="text-base font-bold text-slate-800">Takaslık</span>
                                            <span className="text-xs text-slate-500 font-medium mt-1">Eşyaları değiştir</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-lg font-semibold text-slate-800 mb-1">Eşya Görselleri</h3>
                                    <p className="text-sm text-slate-500 mb-4">Eşyayı en iyi anlatan net fotoğraflar yükleyin. İlk fotoğraf kapak olacaktır.</p>
                                    ...
                                    >
                                        ...
                                        <h4 className="text-base font-bold text-slate-700 mb-1">Fotoğrafları Sürükleyin veya Tıklayın</h4>
                                        <p className="text-sm text-slate-500 text-center font-medium">PNG, JPG (En fazla 5 görsel, max 5MB/adet)</p>
                                        <input
                                            type="file"
                                            accept="image/*"
                                            multiple
                                ...
                                {previews.length > 0 && (
                                    <div className="space-y-3">
                                        <h4 className="text-sm font-semibold text-slate-700">Yüklenenler (Kapak seçmek için tıklayın)</h4>
                                                ...
                                                >
                                                    ...
                                                    {coverIndex === idx && (
                                                        <div className="absolute inset-x-0 bottom-0 bg-emerald-500/90 py-1.5 text-center backdrop-blur-sm">
                                                            <span className="text-[10px] font-bold text-white uppercase tracking-wider">Kapak Fotoğrafı</span>
                                                        </div>
                                                    )}
                        ...
                        {step === 2 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                                <div className="space-y-1.5">
                                    <label className="block text-sm font-semibold text-slate-700">Başlık</label>
                                    <input
                                        {...register('title')}
                                        type="text"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    >
                                        <option value="Diğer">Kategori Seçiniz (İsteğe bağlı)</option>
                                ...
                                {shareType === 'exchange' && (
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-1.5 overflow-hidden">
                                        <label className="block text-sm font-semibold text-emerald-700">Karşılığında Ne Bekliyorsun?</label>
                                        <textarea
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5 relative z-50">
                                        <label className="block text-sm font-semibold text-slate-700">Şehir</label>
                                        <Controller
                                            name="city"
                                            control={control}
                                            render={({ field }) => (
                                    ...
                                    <div className="space-y-1.5 relative z-40">
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <SearchableSelect
                                                    options={selectedCity ? districtsData[selectedCity] : []}
                                                    value={field.value}
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-sm font-semibold text-slate-700">Mahalle <span className="text-slate-400 font-normal text-xs">(İsteğe Bağlı)</span></label>
                                    <input
                        ...
                        {step === 3 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                {shareType === 'donation' && (
                                    <>
                                        <div className="space-y-3">
                                            <label className="block text-sm font-semibold text-slate-700">Yeni Sahibi Belirleme Yöntemi</label>
                                            <div className="grid grid-cols-2 gap-4">
                                                <label className={`flex flex-col items-center justify-center p-5 border-2 rounded-2xl cursor-pointer transition-all ${selectionType === 'lottery' ? 'border-emerald-500 bg-emerald-50/50 shadow-sm ring-2 ring-emerald-500/20' : 'border-slate-100 hover:border-emerald-200 bg-white'}`}>
                                                    <input type="radio" value="lottery" {...register('selectionType')} className="sr-only" />
                                                    <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mb-3 text-2xl">🎲</div>
                                                    <span className="text-sm font-bold text-slate-800 mb-1">Çekiliş Usulü</span>
                                                    <span className="text-xs text-slate-500 text-center px-2">Sistem adil bir şekilde rastgele seçer</span>
                                                </label>
                                                <label className={`flex flex-col items-center justify-center p-5 border-2 rounded-2xl cursor-pointer transition-all ${selectionType === 'manual' ? 'border-emerald-500 bg-emerald-50/50 shadow-sm ring-2 ring-emerald-500/20' : 'border-slate-100 hover:border-emerald-200 bg-white'}`}>
                                                    <input type="radio" value="manual" {...register('selectionType')} className="sr-only" />
                                                    <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-3 text-2xl">👆</div>
                                                    <span className="text-sm font-bold text-slate-800 mb-1">Manuel Seçim</span>
                                                    <span className="text-xs text-slate-500 text-center px-2">Adayları inceleyerek bizzat seçersiniz</span>
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        {selectionType !== 'manual' && (
                                            <div className="space-y-1.5 mt-2 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                                <label className="block text-sm font-semibold text-slate-700">Çekiliş Tarihi</label>
                                                <p className="text-xs text-slate-500 mb-3 font-medium">Çekilişin sistem tarafından otomatik yapılacağı zamanı belirleyin.</p>
                                                <input
                                                    {...register('drawDate')}
                                                    type="datetime-local"
                                                ...
                                                />
                                                {errors.drawDate && <p className="text-xs text-red-500 font-bold mt-1.5">{errors.drawDate.message}</p>}
                                            </div>
                                        )}
                                    </>
                                )}
                                ...
                                {shareType === 'exchange' && (
                                    <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-2xl flex gap-4 items-center">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-emerald-800 mb-1">Takas Şartları</h4>
                                            <p className="text-sm text-emerald-700 leading-snug">Takas gönderilerinde çekiliş sistemi bulunmaz. Paylaşımınıza gelen teklifleri bizzat inceleyerek yeni sahibini kendiniz kararlaştırırsınız.</p>
                                        </div>
                                    </div>
                                )}

                                <div className="space-y-3">
                                    <label className="block text-sm font-semibold text-slate-700">Teslimat Yöntemleri</label>
                                    <div className="grid grid-cols-1 gap-3">
                                        {[
                                            { id: 'pickup', icon: '📍', label: 'Elden Teslim', desc: 'Yeni sahibi belirttiğiniz konuma gelip teslim alır.' },
                                            { id: 'shipping_buyer', icon: '📦', label: 'Kargo (Alıcı Öder)', desc: 'Kargo masraflarını eşyayı teslim alan karşılar.' },
                                            { id: 'shipping_seller', icon: '🎁', label: 'Kargo (Ben Öderim)', desc: 'İyiliğinizi taçlandırın, kargoyu siz karşılayın.' }
                                        ].map(method => (
                                            <label key={method.id} className={`flex items-start p-4 border rounded-2xl cursor-pointer transition-all group ${selectedMethods?.includes(method.id) ? 'border-emerald-500 bg-emerald-50/30 ring-1 ring-emerald-500' : 'border-slate-200 hover:border-emerald-300 bg-white'}`}>
                                                <div className="pt-0.5">
                                                    <input
                                                        type="checkbox"
                                                        value={method.id}
                                                        {...register('deliveryMethods')}
                                                        className="w-5 h-5 text-emerald-600 rounded-md focus:ring-emerald-500 border-slate-300 mt-1 cursor-pointer"
                                                    />
                                                </div>
                                    ...
                                    {errors.deliveryMethods && <p className="text-xs text-red-500 font-medium pl-1">{errors.deliveryMethods.message}</p>}
                                </div>
                            </motion.div>
                        )}
                    </div>
