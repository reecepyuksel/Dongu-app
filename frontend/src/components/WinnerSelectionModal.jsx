...
import { X, Trophy, Dices, UserCheck, Loader2, Star, Gift, Users, Sparkles } from 'lucide-react';
...
// Onay Modalı
// ─────────────────────────────────────────
const ConfirmModal = ({ isOpen, onConfirm, onCancel, selectedUser, isRandom, processing }) => {
                    ...
                    <div className="p-8 text-center">
                        ...
                        <p className="text-slate-500 mb-6">
                            {isRandom
                                ...
                                : (
                                    <>
                                        <span className="font-bold text-slate-700">{selectedUser?.fullName}</span> isimli kullanıcıyı
                                        eşyanın yeni sahibi olarak onaylıyor musunuz?
                                    </>
                                )
                            }
                        </p>
                            ...
                            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mb-6 flex items-center gap-4">
                                ...
                                <div className="text-left">
                                    ...
                                    <div className="flex items-center gap-1 mt-0.5">
                                        ...
                                        <span className="text-xs text-slate-500">{selectedUser.karmaPoint} İyilik Puanı</span>
                                    </div>
                                </div>
                            </div>
                        )}
...
const ParticipantPanel = ({ itemId, itemTitle, selectionType, onSuccess }) => {
    ...
    const [confirmModal, setConfirmModal] = useState({ open: false, user: null, isRandom: false });
    ...
    const fetchApplicants = async () => {
        ...
        try {
            ...
            const sortedApplicants = res.data.sort((a, b) => b.karmaPoint - a.karmaPoint);
    ...
    const handleSelectWinner = async () => {
        ...
        try {
            ...
            showToast(`🎉 Yeni sahibi belirlendi: ${res.data.winner.fullName}`, 'success');
            if (onSuccess) onSuccess(res.data.winner);
        } catch (err) {
            console.error(err);
            showToast(err.response?.data?.message || 'Seçim yapılırken hata oluştu.', 'error');
        } finally {
            setProcessing(false);
            closeConfirm();
        }
            ...
            <div className="p-6 space-y-5">
                {loading ? (
                    ...
                    <div className="text-center py-16">
                        ...
                        <h4 className="font-bold text-slate-600 text-lg mb-1">Henüz katılımcı yok</h4>
                        <p className="text-sm text-slate-400">Bu döngüye henüz kimse katılmadı.</p>
                    </div>
                ) : (
                    <>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="relative">
                                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-bold text-lg shadow-md shadow-emerald-100">
                                                {app.avatarUrl ? (
                                                    <img src={app.avatarUrl} alt={app.fullName} className="w-12 h-12 rounded-full object-cover" />
                                                ) : (
                                        ...
                                        {/* Info */}
                                        <div>
                                            <Link to={`/profile/${app.userId}`} className="font-bold text-slate-800 text-base hover:text-emerald-600 transition-colors inline-block" target="_blank" rel="noopener noreferrer">
                                                {app.fullName}
                                            </Link>
                                            <div className="flex items-center gap-3 mt-1">
                                                <div className="flex items-center gap-1">
                                                    <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                                                    <span className="text-xs font-medium text-slate-500">{app.karmaPoint} İyilik Puanı</span>
                                                </div>
                                                <span className="text-slate-300">•</span>
                                                <span className="text-xs text-slate-400">
                                                    {new Date(app.appliedAt).toLocaleDateString('tr-TR')} tarihinde katıldı
                                                </span>
                                            </div>
                                        </div>
                                    </div>
...
const WinnerSelectionModal = ({ isOpen, onClose, itemId, itemTitle, onSuccess }) => {
    if (!isOpen) return null;
                ...
                >
                    <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50">
                        <div>
                            ...
                            <p className="text-sm text-slate-500 mt-1">"{itemTitle}" için seçim yap</p>
                        </div>
