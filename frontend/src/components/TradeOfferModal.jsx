...
const TradeOfferModal = ({ isOpen, onClose, targetItemId, targetItemTitle }) => {
    const { user } = useAuth();
    ...
    const handleSendOffer = async () => {
        ...
        try {
            ...
            if (offerType === 'text' && (photoFile || videoFile)) {
                payload = new FormData();
                payload.append('targetItemId', targetItemId);
                if (offeredItemId) payload.append('offeredItemId', offeredItemId); // If somehow both are active
            ...
            await api.post(`/messages/trade-offer/send`, payload, config);
            ...
            showToast(error.response?.data?.message || 'Takas teklifi gönderilemedi.', 'error');
        } finally {
            setSending(false);
        }
    };
                ...
                >
                    <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-emerald-50/50">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-slate-800 tracking-tight">Takas Teklifi Et</h2>
                                <p className="text-xs font-medium text-slate-500 truncate max-w-[250px]">
                                    <span className="text-slate-700 font-bold">{targetItemTitle}</span> için bir döngü başlat
                                </p>
                            </div>
                        </div>
                        <button
                    ...
                    <div className="p-6 overflow-y-auto max-h-[60vh]">
                        <div className="flex bg-slate-100 p-1 mb-5 rounded-xl">
                            <button
                                ...
                                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                                    offerType === 'item' ? 'bg-white shadow-sm text-emerald-700' : 'text-slate-500 hover:text-slate-700'
                                }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                                    offerType === 'text' ? 'bg-white shadow-sm text-emerald-700' : 'text-slate-500 hover:text-slate-700'
                                }`}
                            >
                        ...
                        {offerType === 'item' ? (
                            <>
                                ...
                                {loading ? (
                                    <div className="flex flex-col items-center justify-center p-8 text-emerald-600">
                                        ...
                                        <span className="font-bold text-sm">Eşyaların hazırlanıyor...</span>
                                    </div>
                                ) : activeItems.length === 0 ? (
                                    <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-100">
                                        <span className="text-3xl mb-3 block">📦</span>
                                        <h3 className="font-bold text-slate-800 mb-1">Henüz eşya eklememişsin</h3>
                                        <p className="text-sm text-slate-500 mb-4">Mevcut bir eşya seçmek yerine sekme değiştirerek doğrudan teklifinizi yazabilirsiniz.</p>
                                    </div>
                                ) : (
                                        ...
                                        {activeItems.map((item) => (
                                            ...
                                            >
                                                <div className="w-16 h-16 rounded-xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
                                                    <img src={item.imageUrl || 'https://via.placeholder.com/150'} alt={item.title} className="w-full h-full object-cover" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-bold text-slate-800 truncate text-sm">{item.title}</h4>
                                                    <p className="text-xs text-slate-500 truncate mt-0.5">{item.category} • {item.city}</p>
                                                </div>
                                                <div className="pr-3">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                                                        selectedItemId === item.id ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300'
                                                    }`}>
                                                        {selectedItemId === item.id && <div className="w-2 h-2 rounded-full bg-white" />}
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            <div className="animate-fade-in">
                                <p className="text-sm text-slate-600 font-medium mb-2">
                                    Takas için vereceğiniz eşyayı veya sunduğunuz şartları buraya yazın:
                                </p>
                                <textarea
                                ...
                                <div className="flex bg-amber-50 rounded-xl p-3 border border-amber-100 items-start gap-3 mt-4">
                                    ...
                                    <p className="text-xs text-amber-800 font-medium">Bu teklifi gönderdikten sonra aranızda direkt bir sohbet başlar. Fotoğraf yüklemek isterseniz veya daha fazla detayı sonrasında konuşarak tamamlarsınız.</p>
                                </div>
                                    ...
                                    {!photoPreview ? (
                                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 border-dashed rounded-xl cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition group">
                                            <div className="flex flex-col items-center justify-center text-center px-2">
                                                ...
                                                <p className="mb-1 text-sm font-bold text-slate-600">Fotoğraf Yükle</p>
                                            </div>
                                        ...
                                        <div className="relative w-full h-32 rounded-xl overflow-hidden group border border-slate-200 shadow-sm bg-black">
                                            <img src={photoPreview} alt="Fotoğraf Önizleme" className="w-full h-full object-contain" />
                                    ...
                                    {!videoPreview ? (
                                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 border-dashed rounded-xl cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition group">
                                            <div className="flex flex-col items-center justify-center text-center px-2">
                                                ...
                                                <p className="mb-1 text-sm font-bold text-slate-600">Video Yükle</p>
                                            </div>
                                            <input type="file" className="hidden" accept="video/*" onChange={handleVideoChange} />
                                        </label>
                                    ) : (
                                        <div className="relative w-full h-32 rounded-xl overflow-hidden group border border-slate-200 shadow-sm bg-black">
                                            <video src={videoPreview} className="w-full h-full object-contain" muted autoPlay loop />
                                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                                <label className="cursor-pointer bg-white text-slate-800 px-3 py-1.5 rounded-xl text-xs font-bold shadow-lg hover:scale-105 transition-transform">
                                                    Değiştir
                                                    <input type="file" className="hidden" accept="video/*" onChange={handleVideoChange} />
                        ...
                        <div className="mt-6 flex items-center justify-center text-center p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                            <p className="text-sm font-bold text-emerald-700">⭐ Başarılı her takas size <span className="text-xl">+100</span> İyilik Puanı kazandırır!</p>
                        </div>
                    </div>
                    ...
                    <div className="p-5 border-t border-slate-100 bg-slate-50/80 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            disabled={(offerType === 'item' && !selectedItemId) || (offerType === 'text' && !manualOfferText.trim()) || sending}
