...
export const ItemCard = ({
    ...
    // Calculate if ended
    const isEnded = status === 'GIVEN_AWAY' || status === 'DRAW_PENDING' || (drawDate && new Date(drawDate) < new Date());
    
    // Time ago string
    const timeAgoStr = drawDate ? formatDistanceToNow(new Date(drawDate), { locale: tr }) : '';
    ...
    return (
        ...
        >
                ...
                <div className="absolute top-4 left-0 w-full px-4 flex justify-between items-start z-30 pointer-events-none">
                    ...
                    <div className="flex flex-col gap-2 items-start pointer-events-auto">
                        ...
                        {selectionType && shareType !== 'exchange' && (
                            <div className={`bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold shadow-sm flex items-center gap-1 ${selectionType === 'manual' ? 'text-violet-700' : 'text-purple-700'}`}>
                                <span>{selectionType === 'manual' ? '👆 Döngüde' : '🎲 Çekiliş'}</span>
                            </div>
                        )}
                        ...
                        {isEnded && (
                            <div className="bg-slate-800/80 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-slate-200 flex items-center gap-1.5 shadow-sm border border-slate-600">
                                ...
                                <span>{timeAgoStr ? `${timeAgoStr} önce bitti` : 'Tamamlandı'}</span>
                            </div>
                        )}
                    </div>
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                {deliveryMethods && deliveryMethods.length > 0 && deliveryMethods.includes('pickup') && (
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-2 truncate font-[Outfit]">{title}</h3>
                ...
                <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-2">
                        ...
                        {ownerName && (
                            <span className="text-[11px] font-bold text-slate-700">{ownerName}</span>
                        )}
                    </div>
