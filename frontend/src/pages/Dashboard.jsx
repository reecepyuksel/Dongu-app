...
import { Package, Trophy, Clock, Gift, ChevronRight, Star, Truck, MessageCircle, Plus, Heart, Share2 } from 'lucide-react';
    ...
    const handleDeliveryUpdate = async (itemId, status) => {
        try {
            ...
            setMyApplications(prev => prev.map(app =>
                app.item?.id === itemId ? { ...app, item: { ...app.item, deliveryStatus: status } } : app
            ));
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
            ...
            >
                ...
                <div className="lg:col-span-1 flex flex-col items-center justify-center gap-4 bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 text-center">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 font-[Outfit]">{user?.fullName}</h1>
                ...
                <div className="lg:col-span-2 relative bg-white rounded-3xl p-6 md:p-8 shadow-lg overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row justify-between gap-8 h-full">
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-slate-800 font-[Outfit] leading-none mb-0.5">Döngü İstatistiklerin</h2>
                                    <p className="text-slate-400 text-xs font-medium">İyilik yolculuğundaki ilerlemen</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-end justify-between mb-4">
                                <div>
                                    <p className="text-xs text-slate-400 font-medium mb-1">Mevcut Puan</p>
                                    <motion.span 
                                ...
                                <div className="text-right">
                                    <p className="text-xs text-slate-400 font-medium mb-1">Sıradaki Hedef</p>
                            ...
                            {(() => {
                                const kp = user?.karmaPoint || 0;
                                let badgeName = 'Yeni Paylaşımcı', badgeEmoji = '🥉', badgeColor = 'from-amber-100 to-amber-200 text-amber-800';
                                let nextName = 'İyilik Yolcusu', pointsLeft = 251 - kp;
                                if (kp > 2000) { badgeName = 'Döngü Ustası'; badgeEmoji = '💎'; badgeColor = 'from-emerald-100 to-teal-200 text-emerald-800'; nextName = null; pointsLeft = 0; }
                                else if (kp >= 751) { badgeName = 'İyilik Elçisi'; badgeEmoji = '🥇'; badgeColor = 'from-yellow-100 to-amber-200 text-yellow-800'; nextName = 'Döngü Ustası'; pointsLeft = 2001 - kp; }
                                else if (kp >= 251) { badgeName = 'İyilik Yolcusu'; badgeEmoji = '🥈'; badgeColor = 'from-slate-100 to-slate-200 text-slate-700'; nextName = 'İyilik Elçisi'; pointsLeft = 751 - kp; }
                                ...
                                return (
                                    <>
                                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${badgeColor} shadow-sm mb-4 w-fit`}>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden relative shadow-inner cursor-pointer">
                                                    <motion.div
                                                        ...
                                                        className={`absolute h-full rounded-full ${
                                                            kp < 251 ? 'bg-gradient-to-r from-amber-400 to-amber-500' :
                                                            kp < 751 ? 'bg-gradient-to-r from-slate-400 to-slate-500' :
                                                            kp < 2000 ? 'bg-gradient-to-r from-yellow-400 to-amber-500' :
                                                ...
                                                {nextName && (
                                                    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-lg shadow-sm group-hover/bar:scale-110 transition-transform duration-300">
                                                        {nextName === 'İyilik Yolcusu' ? '🥈' : nextName === 'İyilik Elçisi' ? '🥇' : '💎'}
                                                    </div>
                                                )}
                                            </div>

                                            {/* Hover Tooltip */}
                                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[11px] font-medium px-4 py-2 rounded-xl shadow-xl opacity-0 group-hover/bar:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-20">
                                                {nextName 
                        ...
                        <div className="flex flex-col items-start md:items-end justify-between border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8">
                            ...
                            <div className="flex gap-4 mb-6 md:mb-0 w-full md:w-auto">
                                <div className="flex-1 md:flex-none flex flex-col items-center md:items-end bg-slate-50 md:bg-transparent p-3 md:p-0 rounded-xl">
                                    <span className="text-3xl font-bold text-slate-800 mb-1">{myItems.length}</span>
                                    <span className="text-[10px] uppercase tracking-widest text-slate-400 font-medium text-center md:text-right">Döngüye Katılan<br/>Eşya</span>
                                </div>
                                <div className="w-px bg-slate-100 hidden md:block"></div>
                                <div className="flex-1 md:flex-none flex flex-col items-center md:items-end bg-slate-50 md:bg-transparent p-3 md:p-0 rounded-xl">
                                    <span className="text-3xl font-bold text-slate-800 mb-1">
                                        {myApplications.filter(a => a.status === 'WON' || a.item?.winner?.id === user?.id).length}
                                    </span>
                                    <span className="text-[10px] uppercase tracking-widest text-slate-400 font-medium text-center md:text-right">Yeni Sahibi<br/>Olduğun</span>
                                </div>
                            </div>
                ...
                >
                    {myItems.length === 0 ? (
                        <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
                            ...
                            <p className="text-slate-500 font-medium">Henüz bir eşya paylaşmadınız.</p>
                            <p className="text-slate-400 text-sm mt-1">Hemen ilk döngünüzü başlatmaya ne dersiniz?</p>
                        </div>
                    ) : (
                        myItems.map(item => (
                            <div
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-3 mt-1">
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${statusColors[item.status] || 'bg-slate-100 text-slate-500'}`}>
                                            ...
                                            <span className="flex items-center gap-1 text-xs text-slate-400">
                                                ...
                                                {format(new Date(item.drawDate || item.createdAt), 'dd MMM yyyy', { locale: tr })}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                {item.status === 'GIVEN_AWAY' && item.deliveryStatus === 'PENDING' && (
                                    <div className="mt-3 pl-[84px]">
                                        {item.deliveryMethods?.every(m => m === 'pickup') ? (
                                            ...
                                            <button
                                                onClick={() => handleDeliveryUpdate(item.id, 'SHIPPED')}
                ...
                >
                    {myFavorites.length === 0 ? (
                        <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
                            ...
                            <p className="text-slate-500 font-medium">Henüz favorilere eklediğiniz bir paylaşım yok.</p>
                            <Link to="/" className="text-emerald-600 hover:underline text-sm mt-2 inline-block">
                                Vitrine göz atın →
                            </Link>
                        </div>
                    ) : (
                ...
                >
                    {myApplications.length === 0 ? (
                        <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
                            ...
                            <p className="text-slate-500 font-medium">Henüz bir çekilişe katılmadınız.</p>
                            <Link to="/" className="text-emerald-600 hover:underline text-sm mt-2 inline-block">
                                Vitrine göz atın →
                            </Link>
                        </div>
                        ...
                        myApplications.map(app => (
                            ...
                            >
                                ...
                                >
                                    <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0">
                                        <img
                                            src={app.item?.imageUrl || 'https://via.placeholder.com/100'}
                                            alt={app.item?.title}
                                            className="w-full h-full object-cover"
                                        />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-bold text-slate-800 truncate">{app.item?.title || 'Bilinmeyen Ürün'}</h3>
                                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                                            ...
                                            <span className="text-xs text-slate-400">
                                                {formatDistanceToNow(new Date(app.appliedAt), { addSuffix: true, locale: tr })}
                                            </span>
                                        </div>
                                    </div>
                                    <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-emerald-500 transition" />
                                    ...
                                    <div className="mt-3 pl-[84px] flex gap-2">
                                        {app.item?.deliveryMethods?.every(m => m === 'pickup') ? (
                                            ...
                                            app.item?.deliveryStatus === 'PENDING' && (
                                                <button
                                                    onClick={() => { setSelectedConfirmItemId(app.item?.id); setConfirmModalOpen(true); }}
                                                    className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold rounded-lg transition flex items-center gap-2"
                                                >
                                                    🤝 Elden Teslim Aldım
                                                </button>
