...
import { Clock, Users, ShieldCheck, Heart, RefreshCw, ArrowLeft, Calendar, MessageCircle, Truck, Trophy, Gift, CheckCircle, Camera, X, ChevronRight, ChevronLeft, Trash2 } from 'lucide-react';
...
import WinnerSelectionModal, { ParticipantPanel } from '../components/WinnerSelectionModal';
...
const ItemDetail = () => {
    ...
    const handleDeliveryUpdate = async (status) => {
        try {
            ...
            const labels = { SHIPPED: 'Kargoya verildi!', DELIVERED: 'Teslim alındı!' };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            showToast('Eşyayı döngüden çıkardık, her zaman yeni şeyler ekleyebilirsin! ✨', 'success');
            navigate('/');
        } catch (err) {
            showToast(err.response?.data?.message || 'İlanı kaldırırken bir hata oluştu.', 'error');
        } finally {
            setDeleting(false);
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center h-[60vh]">
                <div className="animate-pulse text-emerald-600 font-medium">Yükleniyor...</div>
            </div>
        );
    }
    ...
    if (!item) {
        return (
            <div className="flex flex-col justify-center items-center h-[60vh] text-center">
                ...
                <Link to="/" className="mt-4 text-emerald-600 hover:underline">Vitrine Geri Dön</Link>
            </div>
        );
    }
    const timeLeft = item.drawDate ? formatDistanceToNow(new Date(item.drawDate), { locale: tr }) : null;
    ...
    const isEnded = item.status === 'GIVEN_AWAY' || item.status === 'DRAW_PENDING' || (item.drawDate && new Date(item.drawDate) < new Date());
    const timeAgoStr = item.drawDate ? formatDistanceToNow(new Date(item.drawDate), { locale: tr }) : '';

    const isOwner = user?.id === item.owner?.id;
    const isWinner = user?.id === item.winner?.id;
    const canChat = isAuthenticated && !isOwner;

    const isPickupOnly = item.deliveryMethods && item.deliveryMethods.length > 0 &&
    ...
    const currentDeliveryIndex = deliverySteps.findIndex(s => s.key === item.deliveryStatus);
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12 relative">
            ...
            {isOwner && item.status === 'DRAW_PENDING' && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-8 flex flex-col md:flex-row items-center justify-between gap-5 animate-in fade-in slide-in-from-top-4 duration-700 shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-amber-800 text-lg tracking-tight">Süre Doldu!</h3>
                            <p className="text-amber-700 text-sm md:text-base font-medium">Yeni sahibini belirlemek için seçim sonuçlarını onaylamanız gerekiyor.</p>
                        </div>
                    </div>
                    <button
            ...
            {isWinner && item.status === 'GIVEN_AWAY' && (
                <motion.div
                ...
                >
                    <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-16 h-16 rounded-full bg-emerald-100 flex items-center justify-center border-2 border-emerald-200 shadow-inner shrink-0 text-3xl">🎉</div>
                            <div>
                                <h2 className="text-xl md:text-2xl font-extrabold text-emerald-900 font-[Outfit] tracking-tight">Tebrikler, Yeni Sahibi Sizsiniz! 🎉</h2>
                                <p className="text-sm md:text-base text-emerald-700 font-medium leading-tight mt-1">Bu eşyanın yeni yolculuğu sizinle başlıyor. Teslimat için ilan sahibiyle iletişime geçin.</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={() => { setChatPartner(null); setChatOpen(true); }}
            ...
            {isOwner && item.status === 'GIVEN_AWAY' && item.winner && (
                <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center border-2 border-blue-200 shadow-inner shrink-0 text-3xl">✅</div>
                            <div>
                                <h2 className="text-xl md:text-2xl font-extrabold text-blue-900 font-[Outfit] tracking-tight">Yeni Sahibi Belirlendi!</h2>
                                <p className="text-sm md:text-base text-blue-700 font-medium leading-tight mt-1">Döngü tamamlandı. Teslimat sürecini başlatmak için <b>{item.winner.fullName}</b> ile iletişime geçin.</p>
                            </div>
                        </div>
            ...
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-20 items-start">
                ...
                >
                    ...
                    <div className="relative rounded-[16px] p-2 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100/60">
                        <div className={`grid gap-2 md:gap-2 ${item.images?.length > 1 ? 'grid-cols-1 md:grid-cols-4 md:grid-rows-2 h-[300px] md:h-[450px] lg:h-[500px]' : 'grid-cols-1 aspect-[4/3] md:aspect-[16/10]'}`}>
                            ...
                            >
                                <img
                                    src={(item.images && item.images.length > 0) ? item.images[0] : (item.imageUrl || 'https://via.placeholder.com/800x600?text=Görsel+Yok')}
                                    alt={item.title}
                                ...
                                >
                                    <Heart className={`w-5 h-5 transition-colors duration-300 ${isFavorited ? 'fill-red-500 text-red-500' : 'text-slate-400 group-hover/fav:text-red-400'}`} />
                                </button>
                            ...
                            {/* Küçük Görseller */}
                            {item.images && item.images.length > 1 && item.images.slice(1, 5).map((img, idx) => {
                                let colSpanClasses = "md:col-span-1 md:row-span-1";
                                const total = item.images.length;
                                
                                if (total === 2) colSpanClasses = "md:col-span-2 md:row-span-2";
                                else if (total === 3) colSpanClasses = "md:col-span-2 md:row-span-1";
                                else if (total === 4) {
                                    if (idx === 0 || idx === 1) colSpanClasses = "md:col-span-1 md:row-span-1";
                                    else colSpanClasses = "md:col-span-2 md:row-span-1"; // Son resim
                                }

                                const isLastSlot = (total === 5 && idx === 3) || (total > 5 && idx === 3);
                                ...
                                return (
                                    ...
                                    >
                                        <img src={img} alt={`${item.title} ${idx+2}`} className="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110" />
                        ...
                        {item.images && item.images.length > 1 && (
                            ...
                            >
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="opacity-70"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>
                                Galeriyi Aç
                            </button>
                        )}
                    </div>
                    ...
                    {/* Açıklama & Rozetler */}
                    <div className="pe-4">
                        <h2 className="text-2xl font-bold text-slate-900 font-[Outfit] mb-4">Eşya Hakkında</h2>
                    ...
                    {item.shareType === 'exchange' && (
                        <div className="mt-8 p-6 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-[20px] border border-emerald-100 relative overflow-hidden">
                            ...
                            <h2 className="text-xl font-bold text-emerald-900 font-[Outfit] mb-3 flex items-center gap-2">
                                <span className="text-2xl">🔄</span> Bu döngü bir takas teklifi bekliyor
                            </h2>
                            {item.tradePreferences ? (
                                <div className="space-y-2">
                                    <p className="text-sm font-bold tracking-widest text-emerald-600 uppercase">Takas Karşılığında Aranan:</p>
                                    <p className="text-[16px] text-emerald-800 leading-relaxed font-medium whitespace-pre-wrap relative z-10">"{item.tradePreferences}"</p>
                                </div>
                            ) : (
                                <p className="text-[16px] text-emerald-700 leading-relaxed font-medium relative z-10">
                                    Kullanıcı takas için özel bir tercih belirtmemiş. Neler sunabileceğinizi doğrudan kendisine teklif edebilirsiniz.
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    {isOwner && (item.status === 'AVAILABLE' || item.status === 'DRAW_PENDING') && (
                        <div className="mt-4 pt-8 border-t border-slate-100" id="participant-panel">
                            <h2 className="text-2xl font-bold text-slate-900 font-[Outfit] mb-6">Adaylar & Katılımcılar</h2>
                            <ParticipantPanel
                                itemId={id}
                                ...
                                onSuccess={(winner) => {
                                    fetchItem();
                                    showToast(`🎉 Kazanan belirlendi: ${winner.fullName}. Mesajlaşma açılıyor...`, 'success');
                                    setTimeout(() => { setChatPartner(null); setChatOpen(true); }, 800);
                                }}
                            />
                        </div>
                    )}
                ...
                >
                    <div className="bg-white rounded-[24px] p-6 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100/80 transition-shadow hover:shadow-[0_8px_40px_rgb(0,0,0,0.08)]">
                        ...
                        <div className="flex items-center gap-2 mb-5 flex-wrap">
                            <span className={`px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider ${item.status === 'AVAILABLE' ? 'bg-emerald-100 text-emerald-700' : item.status === 'GIVEN_AWAY' ? 'bg-slate-100 text-slate-600' : 'bg-amber-100 text-amber-700'}`}>
                                {item.status === 'AVAILABLE' ? '🟢 Aktif Döngü' : item.status === 'GIVEN_AWAY' ? '✔️ Tamamlandı' : '⏳ Beklemede'}
                            </span>
                            {item.selectionType && (
                                <span className={`px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider flex items-center gap-1.5 ${item.selectionType === 'manual' ? 'bg-violet-100 text-violet-700' : 'bg-purple-100 text-purple-700'}`}>
                                    {item.selectionType === 'manual' ? '👆 Manuel Seçim' : '🎲 Çekiliş Usulü'}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
                            <span className="flex items-center gap-1.5 text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                                <span className="text-emerald-500 text-lg">📍</span> {item.city}, {item.district}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between group cursor-pointer" onClick={() => navigate(`/profile/${item.owner?.id}`)}>
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col justify-center">
                                    <p className="font-bold text-slate-900 text-[17px] group-hover:text-emerald-600 transition-colors tracking-tight leading-none mb-2">{ownerName}</p>
                                    <div className="flex items-center gap-1.5 font-medium text-xs">
                                        <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-md font-black uppercase tracking-wider text-[10px]">Onaylı</span>
                                        <span className="text-slate-500 flex items-center gap-1">Puan: <span className="text-slate-800 font-bold">{item.owner?.karmaPoint || 0}</span></span>
                                    </div>
                                </div>
                            </div>
                        ...
                        {item.deliveryMethods && item.deliveryMethods.length > 0 && (
                            <div className="mb-6">
                                <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-3">Teslimat Seçenekleri</p>
                                <div className="flex flex-col gap-2">
                                    {item.deliveryMethods.includes('pickup') && (
                                        <div className="flex items-center gap-2 text-sm font-medium text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100"><span className="text-base">📍</span> Gel Al (Elden Teslim)</div>
                                    )}
                                    {item.deliveryMethods.includes('shipping_buyer') && (
                                        <div className="flex items-center gap-2 text-sm font-medium text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100"><span className="text-base">📦</span> Kargo (Alıcı Öder)</div>
                                    )}
                                    {item.deliveryMethods.includes('shipping_seller') && (
                                        <div className="flex items-center gap-2 text-sm font-medium text-slate-700 bg-slate-50 p-2.5 rounded-xl border border-slate-100"><span className="text-base">🎁</span> Kargo (Ben Öderim)</div>
                                    )}
                                </div>
                            </div>
                        )}
                        ...
                        {item.shareType !== 'exchange' && (
                            <div className="grid grid-cols-2 gap-y-7 gap-x-4 mb-8">
                                {item.selectionType !== 'manual' && timeLeft && !isEnded && (
                                    <div>
                                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1.5">Kalan Süre</p>
                                        <p className="text-xl font-black text-emerald-600 tabular-nums tracking-tight">{timeLeft}</p>
                                    </div>
                                )}
                                {isEnded && (
                                    <div>
                                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1.5">Durum</p>
                                        <p className="text-base font-black text-slate-800">{timeAgoStr} bitti</p>
                                    </div>
                                )}
                                {item.selectionType === 'manual' && !isEnded && (
                                    <div>
                                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1.5">Seçim Türü</p>
                                        <p className="text-base font-black text-violet-600 flex items-center gap-1.5">✋ Manuel</p>
                                    </div>
                                )}
                                
                                <div>
                                    <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1.5">Aday Sayısı</p>
                        ...
                        {item.status === 'AVAILABLE' && !isEnded && !isOwner && (
                            <AnimatePresence mode="wait">
                                {!isJoined ? (
                                    ...
                                    <motion.div initial={{opacity:0, y: 10}} animate={{opacity:1, y: 0}} className="flex flex-col gap-3 text-center">
                                        {item.shareType !== 'exchange' && (
                        ...
                        {item.status === 'AVAILABLE' && isEnded && !isOwner && item.shareType !== 'exchange' && (
                            <div className="w-full py-4 bg-slate-50 text-slate-500 rounded-xl text-center flex items-center justify-center gap-2 border border-slate-200">
                                ...
                                <span className="font-bold text-sm">Süre doldu, çekiliş bekleniyor.</span>
                            </div>
                        )}
            ...
            {item.shareType === 'exchange' && (
                <div className="mt-12 bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-slate-100">
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-slate-900 font-[Outfit]">Gelen Takas Teklifleri</h3>
                            <p className="text-slate-500 font-medium">Bu ilana gelen tüm takas teklifleri herkese açıktır.</p>
                        </div>
                    </div>
                    ...
                    {loadingOffers ? (
                        ...
                        <div className="text-center py-16 bg-slate-50 border border-slate-100 rounded-3xl">
                            ...
                            <h4 className="text-lg font-bold text-slate-800">Henüz teklif yok</h4>
                            <p className="text-slate-500">İlk teklifi sen yaparak takası başlatabilirsin!</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {publicOffers.map((offer) => (
                                <div key={offer.id} className="bg-slate-50 border border-slate-100 p-5 rounded-2xl flex flex-col gap-4 hover:border-emerald-200 transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-bold overflow-hidden cursor-pointer shrink-0" onClick={() => navigate(`/profile/${offer.sender?.id}`)}>
                                                {offer.sender?.avatarUrl ? (
                                                    <img src={offer.sender.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                                                ) : (
                                                    offer.sender?.fullName?.charAt(0) || '?'
                                                )}
                                            </div>
                                            <div>
                                                <p className="font-bold text-slate-800 text-sm">{offer.sender?.fullName || 'Bilinmeyen Kullanıcı'}</p>
                                                <p className="text-xs text-slate-400 font-medium">{formatDistanceToNow(new Date(offer.createdAt), { locale: tr })} önce</p>
                                            </div>
                                        </div>
                                        {isOwner && offer.sender?.id !== user?.id && (
                                            <button 
                                                onClick={() => {
                                                    setChatPartner(offer.sender);
                                                    setChatOpen(true);
                                                }}
                                                className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 text-xs font-bold rounded-xl transition-colors flex items-center gap-2"
                                            >
                                                <MessageCircle className="w-4 h-4" /> Sohbet Başlat
                                            </button>
                                        )}
                                    </div>
                                    
                                    <div className="p-3 bg-white rounded-xl border border-slate-100 text-sm text-slate-700 leading-relaxed font-medium">
                                        {offer.content}
                                    </div>

                                    <div className="flex gap-2 max-h-32">
                                        {offer.tradeMediaUrl && (
                                            <div className="flex-1 rounded-xl overflow-hidden border border-slate-200 bg-black/5 relative group cursor-pointer" onClick={() => { setCurrentImageIndex(0); setIsLightboxOpen(true); /* Hack for now, implies we overwrite item.images if we wanted full gallery */ }}>
                                                <img src={offer.tradeMediaUrl} className="w-full h-full object-cover" alt="Teklif Foto" />
                                            </div>
                                        )}
                                        {offer.tradeVideoUrl && (
                                            <div className="flex-1 rounded-xl overflow-hidden border border-slate-200 bg-black relative group">
                                                <video src={offer.tradeVideoUrl} className="w-full h-full object-cover opacity-90 group-hover:opacity-100" muted autoPlay loop playsInline />
                                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                                    <div className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white"><Camera className="w-4 h-4" /></div>
                                                </div>
                                            </div>
                                        )}
                                    </div>
            ...
            <ChatModal
                ...
                itemId={id}
                itemTitle={item.title}
                partnerName={chatPartner?.fullName || (isOwner ? item.winner?.fullName : item.owner?.fullName)}
                partnerId={chatPartner?.id || (isOwner ? item.winner?.id : item.owner?.id)}
            />
            ...
            <AnimatePresence>
                {isLightboxOpen && (
                    ...
                    >
                        ...
                        <div className="w-full h-full max-w-6xl mx-auto p-4 md:p-12 flex items-center justify-center" onClick={() => setIsLightboxOpen(false)}>
                            <motion.img
                                initial={{ scale: 0.9, opacity: 0 }}
                                animate={{ scale: 1, opacity: 1 }}
                                transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                                src={(item.images && item.images.length > 0) ? item.images[currentImageIndex] : (item.imageUrl || 'https://via.placeholder.com/800x600?text=Görsel+Yok')}
                                alt={item.title}
                    ...
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <p className="text-slate-500 text-center mb-8 font-medium leading-relaxed">
                                Bu işlem geri alınamaz. İlanınla birlikte tüm başvurular ve mesajlar da silinecektir.
                            </p>
