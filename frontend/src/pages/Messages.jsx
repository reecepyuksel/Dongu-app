...
import { MessageCircle, ArrowLeft, Clock, ChevronRight, Inbox, Trash2 } from 'lucide-react';
...
const Messages = () => {
    ...
    const handleDeleteConversation = async (e, conv) => {
        ...
        if (!confirm(`"${conv.itemTitle}" paylaşımındaki ${conv.otherUser?.fullName || 'kullanıcı'} ile olan konuşmayı silmek istediğinize emin misiniz?`)) return;
    ...
    if (authLoading) {
        return (
            <div className="flex justify-center items-center h-[50vh]">
                <div className="animate-pulse text-emerald-600">Oturum kontrol ediliyor...</div>
            </div>
        );
    }
        ...
        <div className="max-w-2xl mx-auto px-6 py-12">
            ...
            <div className="mb-8">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Mesajlarım</h1>
            ...
            {conversations.length === 0 ? (
                ...
                >
                    ...
                    <h3 className="font-semibold text-slate-700 mb-2">Henüz mesajınız yok</h3>
                    <p className="text-sm text-slate-400 max-w-xs mx-auto">
                        Bir paylaşıma gidip "Sahibiyle İletişime Geç" butonuna tıklayarak ilk mesajınızı gönderin.
                    </p>
                    <button
                        onClick={() => navigate('/')}
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-1">
                                            <h3 className={`truncate text-sm ${conv.unreadCount > 0 ? 'font-bold text-slate-900' : 'font-semibold text-slate-800'}`}>
                                                {conv.otherUser?.fullName || conv.otherUser?.email || 'Kullanıcı'}
                                            </h3>
                                        ...
                                        <p className={`text-sm truncate ${conv.unreadCount > 0 ? 'text-slate-800 font-bold' : 'text-slate-400'}`}>
                                            {conv.lastMessage}
                                        </p>
                                    </div>
            ...
            {selectedChat && (
                <ChatModal
                    ...
                    onClose={() => {
                        ...
                        fetchConversations(); // Kapanınca listeyi güncelle
                    }}
