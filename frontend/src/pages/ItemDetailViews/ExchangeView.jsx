          ...
          >
            {itemPhotos.map((photo, idx) => (
              <div
                ...
                className={`w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 cursor-pointer transition-all ${currentImageIndex === idx ? 'border-2 border-emerald-400' : 'hover:ring-2 hover:ring-emerald-200'}`}
              >
                <img
                  src={photo.url}
                  alt={`Thumb ${idx}`}
        ...
        <div className="lg:col-span-5 flex flex-col gap-5">
          ...
          <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border-0">
            <div className="flex justify-between items-start mb-2">
              <h1 className="text-2xl font-extrabold text-primary tracking-tight font-[Manrope]">
                {item.title}
              </h1>
              <button
            ...
            <div className="p-4 bg-secondary-container/30 rounded-xl mb-6">
              {item.tradePreferences ? (
                <p className="text-on-secondary-container text-sm leading-relaxed">
                  <span className="font-bold">Arananlar:</span>{' '}
                  {item.tradePreferences}
                </p>
              ) : (
                <p className="text-on-secondary-container text-sm leading-relaxed">
            ...
            {item.status === 'AVAILABLE' && !isOwner && (
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-primary to-primary-container text-white rounded-full font-bold text-base shadow-lg hover:shadow-xl transition-all duration-300 active:scale-95 flex items-center justify-center gap-2"
                >
          ...
          <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border-0">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div
                className="w-14 h-14 rounded-full bg-slate-200 object-cover ring-2 ring-emerald-50 flex items-center justify-center font-bold text-slate-500 text-xl overflow-hidden cursor-pointer"
            ...
            <button
              onClick={() => navigate(`/profile/${item.owner?.id}`)}
              className="w-full mt-4 py-2 border-2 border-outline-variant/20 rounded-xl text-sm font-semibold text-primary hover:bg-surface-container transition-colors"
            >
              Profili İncele
            </button>
          </div>
